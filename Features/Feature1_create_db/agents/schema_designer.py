...
_SCHEMA_DESIGN_CACHE = TTLCache(default_ttl=600)
...
def run_schema_designer(plan: SuggestionPlan) -> DatabaseSchema:
    ...
    cache_key = plan.model_dump_json(indent=2)
    raw = _SCHEMA_DESIGN_CACHE.get(cache_key)
    if raw is None:
      raw = chain.invoke({"plan_json": plan.model_dump_json(indent=2)})
      _SCHEMA_DESIGN_CACHE.set(cache_key, raw)
