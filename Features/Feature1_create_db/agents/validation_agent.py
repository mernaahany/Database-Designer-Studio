...
_LLM_VALIDATION_CACHE = TTLCache(default_ttl=600)
...
def _llm_dynamic_validation(
    ...
    cache_key = (schema.model_dump_json(indent=2), domain)
    raw = _LLM_VALIDATION_CACHE.get(cache_key)
    if raw is None:
        try:
            raw = chain.invoke({
        ...
        _LLM_VALIDATION_CACHE.set(cache_key, raw)
