...
_QUERY_GEN_CACHE = TTLCache(default_ttl=600)
...
def run_query_generator(schema: DatabaseSchema) -> QuerySet:
    ...
    cache_key = schema.model_dump_json(indent=2)
    raw = _QUERY_GEN_CACHE.get(cache_key)
    if raw is None:
      raw = chain.invoke({"schema_json": schema.model_dump_json(indent=2)})
      _QUERY_GEN_CACHE.set(cache_key, raw)
