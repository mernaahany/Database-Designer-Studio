...
_SUGGESTION_CACHE = TTLCache(default_ttl=600)
...
def run_suggestion_agent(
    ...
    cache_key = (analysis.model_dump_json(indent=2), rag_context or "")
    raw = _SUGGESTION_CACHE.get(cache_key)
    if raw is None:
        raw = chain.invoke({
            "analysis_json": analysis.model_dump_json(indent=2),
            "rag_context": rag_context or "No similar schemas available.",
        })
        _SUGGESTION_CACHE.set(cache_key, raw)
